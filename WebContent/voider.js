function TreeDrawer(data,canvas,filter_literals){
	for(var i=0;i<data.rootResources.length;i++){
		var mainTR = canvas.insertRow(canvas.rows.length);
		TreeDrawer.drawNode(data.rootResources[i],mainTR,filter_literals);
	}
}

TreeDrawer.drawNode = function(node,tr,filter_literals){
...
//	$(canvasTD).append(table);
	...
	var has_neighbours = TreeDrawer.hasNeighbours(node,filter_literals || false);
	...
	if(has_neighbours){
		...
		for(var i=0;i<node.neighbours.length;i++){
			if(filter_literals && node.neighbours[i].target.type==="literals"){
				continue;
			}
			...
			TreeDrawer.drawNode(edge.target,new_node_table_tr,filter_literals);
		}
	}
...
TreeDrawer.hasNeighbours = function(node,filter_literals){
	if(filter_literals){
		if(node.neighbours){
			for(var i=0;i<node.neighbours.length;i++){
				if (node.neighbours[i].target.type!=="literals"){
					return true;
				}
			}
		}
